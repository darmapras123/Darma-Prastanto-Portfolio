// app/home/Project/ProjectItems/CertificateTOEFL.tsx
"use client";
...
const OVERLAY_H = 96;
const BUTTON_BOTTOM = 104;

export default function CertificateTOEFL({ onViewerOpenChange }: Props) {
  ...
  useEffect(() => { onViewerOpenChange?.(open); }, [open, onViewerOpenChange]);
  ...
  return (
    <figure className="text-left">
      ...
      >
        ...
        {/* Tombol LIHAT DETAIL — sama seperti DOT: border/line hitam, teks hitam */}
        <button
          type="button"
          onClick={() => setOpen(true)}
          aria-label="Lihat detail TOEFL"
          className={[
        ...
        >
          ...
          />
          <span className="relative z-10 transition-colors duration-200 text-black">
            Lihat detail
          </span>
        ...
        <figcaption className="pointer-events-none absolute inset-x-0 bottom-0 z-20 bg-black/50 text-white px-3 h-24 flex flex-col justify-end overflow-hidden pb-2 pt-3">
          <span className="block font-semibold text-[11px] sm:text-sm leading-snug">TOEFL</span>
          <div className="mt-0.5 flex items-center justify-between">
            <span className="block opacity-90 text-[10px] sm:text-xs leading-tight">English Proficiency - Daily Bahasa Inggris</span>
            <span className="ml-2 inline-flex items-center rounded-full px-2 py-[2px] text-[10px] sm:text-xs font-medium bg-white/12 text-white shadow-[0_0_0_1px_rgba(255,255,255,0.16)_inset] backdrop-blur-[1px]">2025 - 2026</span>
          </div>
        </figcaption>
      </div>
      ...
      <ModalBottomSheet
        ...
        ariaLabel="Pratinjau sertifikat TOEFL"
        widthClassName="w-[96vw] sm:w-[84vw]"
        maxWidthClassName="max-w-[1100px]"
        maxHeightClassName="max-h-[100svh] sm:max-h-[90vh]"  // mobile panjang, desktop tetap
      >
        {/* Tombol X (desktop) */}
        <button
          type="button"
          ...
          aria-label="Tutup"
          className="hidden md:inline-flex items-center justify-center absolute top-2 right-2 w-8 h-8 rounded-md bg-red-600 text-white ring-1 ring-red-400/60 shadow-[0_6px_18px_rgba(0,0,0,0.35)] hover:bg-red-500 active:bg-red-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-300 z-20"
        >
        ...
        {/* === WRAPPER LEBAR PENUH MODAL (RELATIVE) === */}
        <div className="relative w-full">
          {/* MOBILE: header & download ikut flow; DESKTOP: absolute LEBIH ATAS */}
          <div className="px-4 pt-4 text-white sm:px-6 sm:pt-3 sm:absolute sm:left-6 sm:top-2 sm:z-20">
            <p className="text-sm sm:text-base font-semibold leading-tight">
              TOEFL – Daily Bahasa Inggris
            </p>
            <p className="mt-0.5 text-xs sm:text-sm opacity-90 leading-tight">
              PT Daily Cipta Dwipa
            </p>
            <div className="mt-2 inline-flex items-center rounded-full px-3 py-1 text-[11px] sm:text-xs font-medium bg-white/10 text-white border border-white/20 backdrop-blur">
              12 Okt 2025&nbsp;–&nbsp;12 Okt 2026
            </div>

            {/* Button Download (biru kehitaman + shimmer) */}
            <div className="mt-2">
              <a
                href="/certificates/Toefl.png"
                download="Toefl.png"
                className={[
              ...
              >
                ...
                <svg aria-hidden viewBox="0 0 24 24" className="w-4 h-4 relative z-10" fill="currentColor">
                  <path d="M12 3a1 1 0 011 1v8.586l2.293-2.293a1 1 0 111.414 1.414l-4.007 4.007a1 1 0 01-2.121 0L6.572 11.707a1 1 0 111.414-1.414L10.28 12.99V4a1 1 0 011-1z" />
          ...
          {/* Konten gambar:
             - MOBILE: pt-0 (header ikut flow).
             - DESKTOP: pt dikurangi jadi 88px supaya header yang lebih atas tetap clear.
             - pb extra untuk hindari terpotong bawah. */}
          <div className="mx-auto w-[90vw] sm:w-full sm:max-w-[560px] md:max-w-[680px] lg:max-w-[760px] pt-0 sm:pt-[88px] sm:pb-6">
            <Image
              src="/certificates/Toefl.png"
              alt="TOEFL Certificate"
              className="w-full h-auto max-h-[80vh] sm:max-h-[78vh] object-contain select-none rounded-xl"
              width={760}
              height={538}
              priority
