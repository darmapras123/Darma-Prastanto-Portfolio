// app/home/Project/ProjectItems/Poster17.tsx
"use client";

import React, { useEffect, useRef, useState } from "react";
import Image from "next/image";
import ModalBottomSheet from "../../../components/ModalBottomSheet";
import POSTER_17_AGS from "@/public/Project/Poster/poster17_ags.png";

type Props = { onViewerOpenChange?: (open: boolean) => void };

const OVERLAY_H = 96; // ≈ h-24, samakan dengan item lain

export default function Poster17({ onViewerOpenChange }: Props) {
  ...
  // Track posisi kursor global (untuk cek setelah modal ditutup)
  useEffect(() => {
  ...
  // Saat modal ditutup, reset hover hanya jika pointer sudah benar2 di luar elemen
  useEffect(() => {
    if (!open) {
  ...
  return (
    <figure className="text-left">
      ...
      >
        ...
        {/* Tombol LIHAT DETAIL — default border putih tipis; fill biru naik; teks hitam saat hover */}
        <button
          type="button"
          onClick={() => setOpen(true)}
          aria-label="Lihat detail Poster 17 Agustus"
          className={[
            ...
            // posisi bottom = OVERLAY_H (96px) + 8px → 104px
            "bottom-[104px]",
          ].join(" ")}
        >
        ...
        >
          ...
          <span className="block opacity-90 text-[10px] sm:text-xs leading-tight">
            desain poster peringatan kemerdekaan.
          </span>
      ...
      <ModalBottomSheet
        ...
        ariaLabel="Pratinjau Poster 17 Agustus"
